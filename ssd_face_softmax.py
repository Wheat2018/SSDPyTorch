...
class SSD(SSDBackbone):
    ...
    def __init__(self, base, classes_num=2, cfg=None, is_cuda=torch.cuda.is_available()):
        super(SSD, self).__init__(base, is_cuda)
        if cfg is None:
            cfg = self.vgg300_cfg
    ...
    def convolution_predictor(self):
        ...
        for (block, box_num) in zip(self.detect_blocks, self.cfg['multi_box_nums']):
            ...
            conf, _ = repeat_conv2d(1, in_channels, box_num*self.classes_num, 3, padding=1, activation=None)
            self.loc += loc
            self.conf += conf
    ...
    def forward(self, x):
        ...
        Return:
            ...
            list of concat outputs from:
                ...
                2: confidence layers, Shape: [batch_num,priors_num, classes_num]
...
class SSDLoss(nn.Module):
    def __init__(self, match_overlap, neg_pos_rate, do_neg_mining=True, variance=None, is_cuda=torch.cuda.is_available()):
    ...
    def forward(self, predictions, targets, conf_gain=1, thresh=0.4):
        ...
        :param predictions: (loc_p, conf_p, priors)
                            ...
                            conf_p, Shape: tensor[batch_num,priors_num,classes_num]
        ...
        batch_num = conf_p.shape[0]
        priors_num = conf_p.shape[1]
        classes_num = conf_p.shape[2]
        ...
        conf_loss_matrix = -torch.log(F.softmax(cat_conf_p).gather(1, conf_t.view(-1, 1)))
        conf_loss_matrix = conf_loss_matrix.view(batch_num, priors_num)
