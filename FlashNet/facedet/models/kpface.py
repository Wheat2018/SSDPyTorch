...
class KPFace(nn.Module):

    def __init__(self, phase, cfg):
        super(KPFace, self).__init__()
        self.phase = phase
        ...
        self.box_head, self.ldmk_head, self.hard_face_head = self.loss_head()

    def loss_head(self):

        box_heads = list()
        ldmk_heads = list()
        hard_face_heads = list()

        box_heads += [InvertedResidual_Head(inp=32, oup=4, kernel=3, stride=1, expand_ratio=1)]
        box_heads += [InvertedResidual_Head(inp=32, oup=4, kernel=3, stride=1, expand_ratio=1)]
        box_heads += [InvertedResidual_Head(inp=32, oup=4, kernel=3, stride=1, expand_ratio=1)]

        ldmk_heads += [InvertedResidual_Head(inp=32, oup=5, kernel=3, stride=1, expand_ratio=1)]
        ldmk_heads += [InvertedResidual_Head(inp=32, oup=5, kernel=3, stride=1, expand_ratio=1)]
        ldmk_heads += [InvertedResidual_Head(inp=32, oup=5, kernel=3, stride=1, expand_ratio=1)]

        hard_face_heads += [InvertedResidual_Head(inp=32, oup=1, kernel=3, stride=1, expand_ratio=1)]
        hard_face_heads += [InvertedResidual_Head(inp=32, oup=1, kernel=3, stride=1, expand_ratio=1)]
        hard_face_heads += [InvertedResidual_Head(inp=32, oup=1, kernel=3, stride=1, expand_ratio=1)]
        return nn.Sequential(*box_heads), nn.Sequential(*ldmk_heads), nn.Sequential(*hard_face_heads)
    ...
    def forward(self, x):
        ...
        for idx, t in enumerate(features):
            ...
            hard_face.append(self.hard_face_head[idx](t).permute(0, 2, 3, 1).contiguous())

        box = torch.cat([o.view(o.size(0), -1) for o in box], 1)
        ldmk = torch.cat([o.view(o.size(0), -1) for o in ldmk], 1)
        hard_face = torch.cat([o.view(o.size(0), -1) for o in hard_face], 1)

        # if self.phase == "test":
        #     wh = wh.exp()
        output = (box.view(batchsize, -1, 4),
                  ldmk.view(batchsize, -1, 5).sigmoid(),
                  hard_face.view(batchsize, -1, 1).sigmoid())
