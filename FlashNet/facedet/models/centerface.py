...
class CenterFace(nn.Module):
    ...
    def __init__(self, phase, cfg):
        ...
        self.ldmk_reg_type = cfg['ldmk_reg_type']
        ...
        if self.use_ldmk:
            self.wh_head, self.cls_head, self.ctr_head, self.ldmk_head = self.loss_head()
        else:
    ...
    def loss_head(self):
        ...
        if self.use_ldmk:
            ...
            if self.ldmk_reg_type=='coord':
                ...
                ldmk_heads += [InvertedResidual_Head(inp=32, oup=5, kernel=3, stride=1, expand_ratio=1)]
                ldmk_heads += [InvertedResidual_Head(inp=32, oup=5, kernel=3, stride=1, expand_ratio=1)]
            return nn.Sequential(*wh_heads), nn.Sequential(*cls_heads), nn.Sequential(*ctr_heads), nn.Sequential(*ldmk_heads)
        else:
    ...
    def forward(self, x):
        ...
        if self.use_ldmk:
            if self.ldmk_reg_type == 'coord':
                ldmk = ldmk.view(batchsize, -1, 10)
            if self.ldmk_reg_type == 'heatmap':
                ldmk = ldmk.view(batchsize, -1, 5)
