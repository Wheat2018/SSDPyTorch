...
from dataset import AnnotationTransform, LandmarkAnnotationTransform, KPFaceDataset, detection_collate_kpface, preproc_kpface
...
target_generator = KPFaceTargetGenerator(stages=len(cfg['net_cfg']['strides']), stride=cfg['net_cfg']['strides'], valid_range=((8, 64), (64, 128), (128, 320)))
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        ...
        # import pdb
        # pdb.set_trace()
        # loss_hard_face = hard_face_criterion(pred=hard_face_preds, mask=None, target=hard_face_targets)
        loss_hard_face = loss_ldmk

        loss = cfg['train_cfg']['box_weight'] * loss_box \
               + cfg['train_cfg']['ldmk_weight'] * loss_ldmk \
               + cfg['train_cfg']['hard_face_weight'] * loss_hard_face
    ...
    torch.save(net.state_dict(), os.path.join(args.save_folder, 'Final_KPFace.pth'))
