...
class KPFaceDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.preproc is not None:
            if self.aug_type == 'FaceBoxes':
                img, box_target, box_mask, ldmk_target, hard_face_target = self.preproc(img, target)
                return torch.from_numpy(img), box_target, box_mask, ldmk_target, hard_face_target


def detection_collate_kpface(batch):
    ...
    for _, (img, box_target, box_mask, ldmk_target, hard_face_target) in enumerate(batch):
        ...
        if torch.is_tensor(hard_face_target):
            # print('ldmk_target',ldmk_target.size())
            hard_face_targets.append(hard_face_target)

    return (torch.stack(imgs, 0),
            torch.stack(box_targets, 0),
            torch.stack(box_masks, 0),
            torch.stack(ldmk_targets, 0),
            torch.stack(hard_face_targets, 0))