...
# 过滤掉最小脸的尺度：大于minsize2select的脸保留
minsize2select = args.minium_face_size
usepadding = True
...
def convertimgset(img_set="train"):
    imgdir = os.path.join(args.dataset_root, "WIDER_" + img_set + "/images")
    gtfilepath = args.dataset_root + "/wider_face_split/wider_face_" + img_set + "_bbx_gt.txt"
    imagesdir = args.dataset_root + "/JPEGImages"
    vocannotationdir = args.dataset_root + "/Annotations"
    labelsdir = args.dataset_root + "/labels"
    if not os.path.exists(imagesdir):
        os.makedirs(imagesdir)
    if not os.path.exists(args.dataset_root + "/ImageSets"):
        os.makedirs(args.dataset_root + "/ImageSets")
    if not os.path.exists(args.dataset_root + "/ImageSets/Main"):
        os.makedirs(args.dataset_root + "/ImageSets/Main")
    ...
    f_set = open(args.dataset_root + "/ImageSets/Main/" + img_set + ".txt", 'w')
...
def generatetxt(img_set="train"):
    gtfilepath = args.dataset_root + "/wider_face_split/wider_face_" + img_set + "_bbx_gt.txt"
    f = open(args.dataset_root + "/" + img_set + ".txt", "w")
    with open(gtfilepath, 'r') as gtfile:
        while (True):  # and len(faces)<10
            filename = gtfile.readline()[:-1]
            if (filename == ""):
...
def generatevocsets(img_set="train"):
    if not os.path.exists(args.dataset_root + "/ImageSets"):
        os.mkdir(args.dataset_root + "/ImageSets")
    if not os.path.exists(args.dataset_root + "/ImageSets/Main"):
        os.mkdir(args.dataset_root + "/ImageSets/Main")
    gtfilepath = args.dataset_root + "/wider_face_split/wider_face_" + img_set + "_bbx_gt.txt"
    f = open(args.dataset_root + "/ImageSets/Main/" + img_set + ".txt", 'w')
    with open(gtfilepath, 'r') as gtfile:
        while (True):  # and len(faces)<10
            filename = gtfile.readline()[:-1]
            if (filename == ""):
...
if __name__ == "__main__":
    convertdataset()
    shutil.move(args.dataset_root + "/ImageSets/Main/" + "train.txt", args.dataset_root + "/ImageSets/Main/" + "trainval.txt")
    shutil.move(args.dataset_root + "/ImageSets/Main/" + "val.txt", args.dataset_root + "/ImageSets/Main/" + "test.txt")