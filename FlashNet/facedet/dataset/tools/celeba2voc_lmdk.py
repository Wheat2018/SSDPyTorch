...
rootdir = "/mnt/lustre/geyongtao/dataset/CelebA"
...
def convertimgset(img_set="train"):
    ...
    imgdir = os.path.join(rootdir, "img_align_celeba")
    gt_box_filepath = os.path.join(rootdir, "list_boxes_align_celeba.txt")
    gt_landmark_path = os.path.join(rootdir, "list_landmarks_align_celeba.txt")
    ...
    with open(gt_box_filepath, 'r') as gt_box_file:
        gt_box_file.readline()
        gt_landmark_file.readline()
        gt_landmark_file.readline()
        while (True):  # and len(faces)<10
            landmark_line = gt_landmark_file.readline().split()
            box_line = gt_box_file.readline().split()
            box_filename = landmark_line[0]
            landmark_filename = box_line[0]
            if not landmark_filename:
                break
            print(box_filename)
            print(landmark_filename)
            imgpath = os.path.join(imgdir, box_filename)
            img = cv2.imread(imgpath)
            # import pdb
            # pdb.set_trace()
            if landmark_filename != box_filename:
                assert("gt landmark and gt bbox is not sharing the same image!")

            bboxes = []
            bboxes_landmark = []

            xmin = int(float(box_line[1]))
            ymin = int(float(box_line[2]))
            xmax = int(float(box_line[3]))
            ymax = int(float(box_line[4]))
            blur = -1
            expression = -1
            illumination = -1
            invalid = -1
            ...
            x1 = float(landmark_line[1])
            y1 = float(landmark_line[2])
            visible1 = float(1.0)
            x2 = float(landmark_line[3])
            y2 = float(landmark_line[4])
            visible2 = float(1.0)
            x3 = float(landmark_line[5])
            y3 = float(landmark_line[6])
            visible3 = float(1.0)
            x4 = float(landmark_line[7])
            y4 = float(landmark_line[8])
            visible4 = float(1.0)
            x5 = float(landmark_line[9])
            y5 = float(landmark_line[10])
            visible5 = float(1.0)
            blur_score = -1
            ...
            path, _ = os.path.split(imagesdir + "/" + box_filename)  # 返回一个路径名和文件名
            if not os.path.exists(path):
                os.makedirs(path)
            if len(bboxes) == 0:
                print("no face:", box_filename)
                continue
            # cv2.imwrite(imagesdir + "/" + box_filename, saveimg)
            imgfilepath = box_filename[:-4]
            f_set.write(imgfilepath + '\n')
            if convert2vocformat:
                xmlpath = vocannotationdir + "/" + box_filename
                ...
                filename_name = doc.createTextNode(box_filename)
