...
#         anchors = [16,32,64,128,256,400]
#         anchors = [12,24,60,100,160, 256]
        anchors = self.anchors
        
        distance = self.infDistance
        anchor_idx = len(self.anchors) - 1
...
#         print('anchor_idx',anchor_idx)
        target_anchor = random.choice(anchors[0:min(anchor_idx+1,len(self.anchors)-1)+1])
#         print('anchor_idx',anchor_idx)
#         print('target_anchor', target_anchor)
...
#         image = cv2.resize(image, fx=ratio, fy=ratio, interpolation=interp_method)
...
#         scales = [16, 24, 60,100,160, 300]

# mode None:w==short_size     0.3 or 0.5:w
        while True:
            ...
            for _ in range(50):
                if mode is None or mode < 0.7:
                  if mode is None:
                    ...
                    w = random_random.randrange(int(0.3*short_size), int(0.5*short_size))
                  h = w
...
#                    print(self.scales)
#                    print('self.scales[3:]',self.scales[3:])
#                    print('self.scales[:-1]',self.scales[:-1])
                   assert len(self.scales) > 5
                   if w > self.scales[-2]:
                       random_scale = self.size / random_random.choice(self.scales[-3:])
                   elif w > self.scales[-3]:
#                        print('self.scales[:-1]',self.scales[:-1])
                       random_scale = self.size / random_random.choice(self.scales[-4:])
                   elif w > self.scales[-4]:
                       random_scale = self.size / random_random.choice(self.scales[-5:])
                   elif w > self.scales[-5]:
                       random_scale = self.size / random_random.choice(self.scales[:-3])
                   elif w > self.scales[-6]:
                       random_scale = self.size / random_random.choice(self.scales[:-4])
                   else:
                       random_scale = self.size / random_random.choice(self.scales[:-5])
...
#                    print('self.random_scale',random_scale)        
                   h = w 
...
class RandomCropLargeScale(object):

    def __init__(self , image_size, anchors):
        self.options = [None, 0.3, 0.5, 0.7, 0.9]
        self.small_threshold = 16.0  
        self.anchors = anchors
        self.size = image_size
        self.scales = self.anchors
...
class SSDAugmentation(object):
    def __init__(self, anchors = [16,32,64,128,256,512], size=300, mean=(104, 117, 123)):
        self.mean = mean
        self.size = size
        self.anchors = anchors
        self.data_anchor_sampling_augment = Compose([
            ConvertFromInts(),
#                 ToAbsoluteCoords(),
            PhotometricDistort(),
            #RandomSampleCrop(),
            RandomBaiduCrop(self.size, self.anchors),
            #Expand(self.mean),
            RandomMirror(),
            ToPercentCoords(),
            Resize(self.size),
        ...
        self.ssd_augment = Compose([
            ConvertFromInts(),
#                 ToAbsoluteCoords(),
            PhotometricDistort(),
            #Expand(self.mean),
            RandomCrop(self.size, self.anchors),
            RandomMirror(),
            ToPercentCoords(),
            Resize(self.size),
            SubtractMeans(self.mean)
...
#         self.size = random_random.choice( [640,832,1024] )
        #hwc
        prob = np.random.uniform(0., 1.)
        if prob > 0.4:
#             print('ssd_augment')
            img, boxes, labels = self.ssd_augment(img, boxes, labels)
        else:
#             print('data_anchor_sampling_augment')
            img, boxes, labels = self.data_anchor_sampling_augment(img, boxes, labels)
