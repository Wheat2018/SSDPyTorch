...
class WarmupStepSchedule(LambdaLR):
    ...
    Example:
        ...
        >>> # lr linearly increase
        >>> # lr = 0.05     if 10 <= epoch < 30
    ...
    def __init__(self, optimizer, warmup_steps, step_size, gamma=0.1):
        ...
        def lr_lambda(step):
            if step < warmup_steps:
                return float(step) / float(max(1.0, warmup_steps))
            elif warmup_steps <= step < step_size:
                return self.gamma
            else:
                return self.gamma ** math.ceil(step / self.step_size)

        super(WarmupStepSchedule, self).__init__(optimizer, warmup_steps, step_size)
